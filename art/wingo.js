...
var jt_AltVu = {
	...
	init: function(wrapID) {
		...
		jt_AltVu.animC = new jt_AnimCalc(true, function() {
			var opacity = jt_AltVu.animC.getVal('opacity');
			jt_setOpacity(jt_AltVu.inDIV, opacity);
			jt_setOpacity(jt_AltVu.outDIV, 100 - opacity);
			}, {'numSteps':15, 'elapsed':750});
		jt_AltVu.animC.setRange('opacity', 0, 100);
		//}
		jt_AddListener(jt_AltVu.qWrapDIV, "mouseover", function() {
			clearTimeout(jt_AltVu.timer);
			jt_AltVu.qWrapDIV.style.background = "#f8f8ff";
			jt_ShowNoneElm(jt_AltVu.btnDIV, true);
			jt_AltVu.mOver = true;
			});
		jt_AddListener(jt_AltVu.qWrapDIV, "mouseout", function() {
			jt_AltVu.qWrapDIV.style.background = "none";
			jt_ShowNoneElm(jt_AltVu.btnDIV);
		...
		jt_ShowNoneElm(jt_AltVu.inDIV, true);
	...
	fade: function() {
		...
		jt_ShowNoneElm(jt_AltVu.inDIV, true);
		if (BrowserDetect.browser == 'Explorer') {
			jt_ShowNoneElm(jt_AltVu.outDIV);
			jt_AltVu.next();
			jt_AltVu.start();
		}
		else {
	...
	setHeight: function() {
		jt_AltVu.qWrapDIV.style.height = jt_valPx(jt_height(jt_AltVu.list[jt_AltVu.idx]));
	},
	next: function() {
	...
	showDIV: function() {
		for (var i=0; i<jt_AltVu.list.length; i++) {
			jt_ShowNoneElm(jt_AltVu.list[i], i==jt_AltVu.idx);
		}
		jt_AltVu.inDIV = jt_AltVu.list[jt_AltVu.idx];
		jt_setOpacity(jt_AltVu.inDIV, 100);
		jt_AltVu.setHeight();
		clearTimeout(jt_AltVu.timer);
	},
	timer: null,
...
var vidBox = {
	...
	play: function(video_id) {
		...
		jt_ShowNoneElm(vidBox.curr, true);
		if (firstTime) vidBox.vu.moveTo(-1, -1);
	},
	clr: function() {
		if (vidBox.curr != null) {
			jt_ShowNoneElm(vidBox.curr);
